...
export class AuthService {
  ...
  createUser(email: string, password: string, name: string, lastName: string) {
    const authData: Auth = { email: email, password: password, name:name, lastName:lastName, photoUrl:''};
    this.http
  ...
  login(email: string, password: string) {
    ...
    this.http
      .post<{ token: string; expiresIn: number, userId:string, role:string, email:string, active:string }>(
        `${baseURL}/login`,
        authData
      )
      .subscribe(response => {
        ...
        if (token) {
          ...
          this.saveAuthData(token, expirationDate, response.userId, response.role, response.email, response.active);
  ...
  private saveAuthData(token: string, expirationDate: Date, userId:string, role:string, email:string, active:string) {
